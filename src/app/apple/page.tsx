...
const Apple = () => {
	...
	useEffect(() => {
		const handleScroll = () => {
			...
			const pastLastProduct = lastProductRef
				? scrollPosition > lastProductRef.getBoundingClientRect().bottom + window.scrollY
				: false;

			const firstProductRef = productRefs.current[0];
			const extraPixels = -600;
			const pastFirstProduct = firstProductRef
				? scrollPosition < firstProductRef.getBoundingClientRect().top + window.scrollY - extraPixels
				: false;
	...
	return (
		<div className='bg-[#050813]'>
			...
			{showStickyCategory && hasScrolled && !isAtBottom && (
				<div
					className={`fixed flex flex-row lg:flex-col gap-3 w-full lg:w-32 top-48 md:top-40 lg:top-1/3 left-0 lg:left-4 z-50 shadow-lg animate-none lg:animate-slide-in ${
						showStickyCategory ? 'opacity-100' : 'opacity-0'
					}`}
				>
					<div className='hidden lg:flex flex-col gap-4'>
						{categories.map((category, index) => (
							<div
								key={index}
								className={`py-4 px-3 rounded-md cursor-pointer ${
									activeCategoryIndex === index ? 'bg-primary' : 'bg-white'
						...
						<Swiper spaceBetween={10} slidesPerView={6}>
							{categories.map((category, index) => (
								<SwiperSlide key={index}>
									<div
										className={`py-2 md:py-4 px-0 md:px-3 w-full rounded-md cursor-pointer ${
											activeCategoryIndex === index ? 'bg-primary' : 'bg-white'
										}`}
										onClick={() => scrollToCategory(index)}
									>
