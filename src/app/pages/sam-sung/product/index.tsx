...
const slidesData = [
    {
        tabs: [
            { label: '', contentHeader: 'SẢN PHẨM MỚI' },
            { label: '', contentHeader: 'SẢN PHẨM MỚI' },
            { label: '', contentHeader: 'SẢN PHẨM MỚI' },
            { label: '', contentHeader: 'SẢN PHẨM MỚI' }
        ],
        content: Array.from({ length: 10 }).map((_, index) => ({
            name: `Sam Sung ${index + 1}`,
            image: `/sam-sung/product-new.png`,
    ...
    },
    {
        ...
        content: Array.from({ length: 10 }).map((_, index) => ({
            name: `iPad ${index + 1}`,
            image: `/sam-sung/product-ipad.png`,
            price: `${(index + 1) * 1000000} VND`
        }))
    },
    {
        tabs: [
            { label: 'sam-sung Watch Series 8', contentHeader: 'WATCH DẪN ĐẦU GIÁ RẺ' },
            { label: 'sam-sung Watch SE', contentHeader: 'WATCH DẪN ĐẦU GIÁ RẺ' },
            { label: 'sam-sung Watch Ultra', contentHeader: 'WATCH DẪN ĐẦU GIÁ RẺ' },
            { label: 'sam-sung Watch Series 7', contentHeader: 'WATCH DẪN ĐẦU GIÁ RẺ' }
        ],
        content: Array.from({ length: 10 }).map((_, index) => ({
            name: `Watch ${index + 1}`,
            image: `/sam-sung/product-watch.jpg`,
            price: `${(index + 1) * 1000000} VND`
        }))
    },
    {
        ...
        content: Array.from({ length: 10 }).map((_, index) => ({
            name: `Mac ${index + 1}`,
            image: `/sam-sung/product-macbook.png`,
            price: `${(index + 1) * 1000000} VND`
        }))
    },
    {
        ...
        content: Array.from({ length: 10 }).map((_, index) => ({
            ...
            image: `/sam-sung/product-airpod.jpeg`,
            price: `${(index + 1) * 1000000} VND`
        }))
    },
    {
    ...
    const renderSwiper = (index: number) => {
        ...
        return (
            <div className='w-full flex flex-col gap-4' ref={(el: HTMLDivElement | null) => { if (el) productRefs.current[index] = el; }}>
                {index === 0 && (
                    <div className='bg-white py-2 px-8 rounded-full w-fit mx-auto shadow-md'>
                        <h3 className='uppercase text-xl text-center font-semibold'> Sản phẩm mới</h3>
                    </div>
                )}
                {index !== 0 && (
                    <>
                        <Image
                            src={`/sam-sung/product-banner-0${index + 1}.png`}
                            width={1820}
                            height={1200}
